...
def create_matrices(molecules_dir, output_file_csv):
    ...
    jar_path = os.path.join(local_directory_path, 'treegraph', 'MatrixConverter.jar')
    ...
    treegraph_script_path = os.path.join(local_directory_path, 'treegraph', 'treeGraphs.py')
    ...
    no_extracted_matrices(molecules_dir, no_extracted_matrix_path, treegraph_script_path)
    # extract matrices
    ...
    print('\x1b[1;32;40m' + f'{output_file_csv} created' + '\x1b[0m')
...
def extract_matrices(no_extracted_matrices_path, output_dir, jar_path):
    ...
    for f in sorted(os.listdir(no_extracted_matrices_path)):
        ...
        matrix = subprocess.run(['java', '-jar', jar_path, f_path], capture_output=True).stdout.decode('utf-8').strip()
        with open(os.path.join(output_dir, f) + '_AdjMat.txt', 'w+') as adj:
            adj.write(matrix)


def no_extracted_matrices(molecules_dir, output_dir, treegraph_script_path):
    print('launching treeGraphs.py...')
    subprocess.run(['python', treegraph_script_path, molecules_dir, output_dir])


parser = argparse.ArgumentParser(description='treegraph tool')
...
print('\x1b[0;31;40m' + 'TREEGRAPH DISTANCE TOOL' + '\x1b[0m')
