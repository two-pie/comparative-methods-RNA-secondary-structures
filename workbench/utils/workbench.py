...
if __name__ == '__main__':
    ...
    # Archaea
    subprocess.run(['aspralign_distance_tool', dbnh_archaea, result_MP_aspralign_Archaea])
    ...
    subprocess.run(['rnaforester_distance_tool', dbnh_archaea, result_MP_rnaforester_Archaea])
    subprocess.run(['treegraph_distance_tool', ctnh_archaea, result_MP_treegraph_Archaea])
    # Bacteria
    subprocess.run(['aspralign_distance_tool', dbnh_bacteria, result_MP_aspralign_Bacteria])
    ...
    subprocess.run(['rnaforester_distance_tool', dbnh_bacteria, result_MP_rnaforester_Bacteria])
    subprocess.run(['treegraph_distance_tool', ctnh_bacteria, result_MP_treegraph_Bacteria])
    # Eukaryota
    subprocess.run(['aspralign_distance_tool', dbnh_eukaryota, result_MP_aspralign_Eukaryota])
    ...
    subprocess.run(['rnaforester_distance_tool', dbnh_eukaryota, result_MP_rnaforester_Eukaryota])
    subprocess.run(['treegraph_distance_tool', ctnh_eukaryota, result_MP_treegraph_Eukaryota])
    # Core calculator
    subprocess.run(['python3', os.path.join(WORKBENCH_PATH, 'utils', 'cores_calculator_workbench.py')])
    # Clustering calculator
    subprocess.run(['python3', os.path.join(WORKBENCH_PATH, 'utils', 'cluster_matrix_workbench.py')])
    subprocess.run(['python3', os.path.join(WORKBENCH_PATH, 'utils', 'cluster_features_workbench.py')])
